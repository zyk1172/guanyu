          ...
          <div className="space-y-1">
            ...
            >
              {REPORT_LANGUAGE_OPTIONS.map((option) => (
                <option key={option.value} value={option.value}>{option.value === 'en-US' ? t('report.englishOption') : t('report.chineseOption')}</option>
              ))}
            </select>
            <p className="text-xxs text-gray-400">{t('report.outputLanguageHint')}</p>
          </div>
        </div>
