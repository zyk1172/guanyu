...
export default function PlatformModelAdmin() {
  ...
  const action = async (model: AdminModel, actionName: 'test' | 'setDefault' | 'delete') => {
    ...
    try {
      ...
      setMessage(actionName === 'test' ? (data.passed ? t('platformAdmin.testPassed', '模型连接测试通过。') : t('platformAdmin.testFailed', '模型连接测试失败。')) : t('platformAdmin.actionDone', '操作已完成。'));
      await load();
    ...
    <section className="rounded-xl border border-[var(--color-border)] bg-[var(--color-card)] p-4 shadow-[var(--shadow-card)] sm:p-5">
      ...
      <div className="mt-3 grid gap-2 lg:grid-cols-2">
        {models.map((model) => {
          ...
          return <article key={model.id} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-3">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5"><button type="button" onClick={() => { setDraft(toDraft(model)); setExpanded(true); }} className="rounded border border-[var(--color-border)] px-2 py-1 text-xxs font-black text-[var(--color-text)]">{t('platformAdmin.edit', '编辑')}</button><button type="button" disabled={Boolean(busy)} onClick={() => void action(model, 'test')} className="rounded border border-[var(--color-border)] px-2 py-1 text-xxs font-black text-[var(--color-link)]">{busy === `test:${model.id}` ? t('platformAdmin.testing', '测试中…') : t('platformAdmin.test', '连接测试')}</button>{!model.isDefault && <button type="button" onClick={() => void action(model, 'setDefault')} className="rounded border border-[var(--color-border)] px-2 py-1 text-xxs font-black text-[var(--color-success)]">{t('platformAdmin.setDefault', '设为默认')}</button>}{!model.isDefault && <button type="button" onClick={() => void action(model, 'delete')} className="rounded border border-[var(--color-danger)]/40 px-2 py-1 text-xxs font-black text-[var(--color-danger)]">{t('common.delete', '删除')}</button>}</div>
            {model.lastTestedAt && <p className="mt-2 text-xxs text-[var(--color-text-subtle)]">{model.lastTestMessage} · {new Date(model.lastTestedAt).toLocaleString(language)}</p>}
          </article>;
        })}
      </div>
      ...
      {expanded && <div className="mt-4 rounded-xl border border-[var(--color-border-strong)] bg-[var(--color-surface-muted)] p-4">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Field label={t('platformAdmin.provider', '服务商')}><select className={inputClass} value={draft.provider} onChange={(e) => setDraft({ ...draft, provider: e.target.value, searchMode: e.target.value === 'openai_compatible' && draft.searchMode === 'native' ? 'platform' : draft.searchMode })}><option value="openai_compatible">OpenAI-compatible</option><option value="openai">OpenAI</option><option value="gemini">Gemini</option><option value="anthropic">Claude / Anthropic</option></select></Field>
