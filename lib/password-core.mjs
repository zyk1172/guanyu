...
export function assertValidPasswordInput(password) {
  ...
  if (bytes < 8) throw new Error('密码至少需要 8 个字节。');
...
function isValidPasswordInput(password) {
  ...
  return bytes >= 8 && bytes <= MAX_PASSWORD_BYTES;
}

export async function hashPassword(password) {
  assertValidPasswordInput(password);
  const salt = randomBytes(16).toString('base64url');
  const digest = Buffer.from(await scrypt(password, salt, 64)).toString('base64url');
  return `${PASSWORD_PREFIX}$${salt}$${digest}`;
}

export async function verifyPassword(password, storedValue) {
  if (!isValidPasswordInput(password) || typeof storedValue !== 'string') {
