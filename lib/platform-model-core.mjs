...
export function effectivePlatformModelId(models, preferredId) {
  ...
  if (preferred && available.some((item) => item.id === preferred)) return preferred;

  return String(
    available.find((item) => item.isUserDefault)?.id
    || available.find((item) => item.isDefault)?.id
    || available[0]?.id
    || '',
  );
}
...
export function isOfficialOpenAiApiBaseUrl(value) {
  try {
    const url = new URL(String(value || ''));
    const hostname = url.hostname.toLowerCase();
    const officialHost = hostname === 'api.openai.com' || hostname.endsWith('.api.openai.com');
    const officialPath = url.pathname === '/v1' || url.pathname === '/v1/';
