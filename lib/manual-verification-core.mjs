...
function defaultReadingReason(language) {
  return String(language || '').startsWith('zh')
    ? '该判断综合信息完整度、证据强度、叙事倾向性和待核验问题得出。'
    : 'This judgment combines information completeness, evidence strength, narrative steering, and unresolved verification questions.';
}

function isLegacyVerificationReason(value, language) {
  const text = String(value || '').trim();
  return String(language || '').startsWith('zh')
    ? text.startsWith('已根据 ') && text.includes('用户核验标记重新计算')
    : text.startsWith('Recalculated from ') && text.includes('user verification marks');
}

function baseReadingReason(report, language) {
  const saved = String(report?.readingValueBaseReason || '').trim();
  if (saved) return saved;
  const current = String(report?.readingValueReason || '').trim();
  return current && !isLegacyVerificationReason(current, language) ? current : defaultReadingReason(language);
}

function isReadWorthLabel(value) {
  return Object.hasOwn(READ_WORTH_RANK, value);
}

function currentReadWorth(report, scores) {
  const stored = report?.read_worth?.label || report?.readingValue;
  if (isReadWorthLabel(stored)) return { label: stored, score: Number(report?.read_worth?.score) || 0 };
  return computeReadWorthCore({ scores });
}
...
export function applyManualVerification(report, { index, outcome, updatedAt = new Date().toISOString() } = {}) {
  ...
  const previousScores = pickScores(next);
  const previousReadWorth = currentReadWorth(next, previousScores);
  const scores = {
  ...
  const recalculatedReadWorth = computeReadWorthCore({ scores });
  // Finding material for a verification target must not downgrade the reading
  // recommendation. The scores still change, but the label remains monotonic.
  const readWorth = outcome === 'verified' && READ_WORTH_RANK[recalculatedReadWorth.label] < READ_WORTH_RANK[previousReadWorth.label]
    ? { ...recalculatedReadWorth, label: previousReadWorth.label, score: Math.max(recalculatedReadWorth.score, previousReadWorth.score) }
    : recalculatedReadWorth;
  ...
  const originalReadingReason = baseReadingReason(next, language);
  ...
  next.readingValueBaseReason = originalReadingReason;
  next.readingValueReason = originalReadingReason;
  next.readingValueVerificationReason = reasonText(manualVerifications, language);
