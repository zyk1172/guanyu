...
function reasonText(records, language) {
  ...
  if (chinese) {
    return `已根据 ${records.length} 条用户核验标记更新证据状态：${verified} 条标记为已找到可核验材料，${unverified} 条标记为暂未找到可核验材料。核心指数和阅读提示已调整；阅读价值档位仍由信息收益决定。`;
  }
  return `Evidence status was updated from ${records.length} user verification marks: ${verified} marked as checkable material found and ${unverified} as not found. Core indicators and reading guidance were adjusted; the reading-value tier remains based on information utility.`;
}

function defaultReadingReason(language) {
  return String(language || '').startsWith('zh')
    ? '阅读价值依据公共重要性、信息增量、独特性、解释深度、行动参考价值和信息密度判断。'
    : 'Reading value is based on public importance, information gain, uniqueness, explanatory depth, actionability, and information density.';
}
...
export function applyManualVerification(report, { index, outcome, updatedAt = new Date().toISOString() } = {}) {
  ...
  // Manual verification changes evidence quality and reading guidance only.
  // The reading-value tier remains based on the report's stable utility factors.
  const readWorth = computeReadWorthCore({
    scores,
    readingUtility: next.readingUtility || next.reading_utility,
    readingValue: next.read_worth?.label || next.readingValue,
