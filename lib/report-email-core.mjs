...
function bulletHtml(items, formatter = (item) => asText(item)) {
  const content = asItems(items).map(formatter).filter(Boolean);
  if (!content.length) return '<p style="margin:0;color:#697785">—</p>';
  return `<ul style="margin:10px 0 0;padding-left:20px;color:#3d4b58;line-height:1.75">${content.map((item) => `<li style="margin:5px 0">${escapeHtml(item)}</li>`).join('')}</ul>`;
}
...
function sectionHtml(title, body, accent = '#345a73') {
  return `<section style="margin:28px 0 0;padding:0 0 24px;border-bottom:1px solid #e3e8ec">
    <h2 style="margin:0 0 12px;color:${accent};font-size:17px;line-height:1.4;font-weight:700">${escapeHtml(title)}</h2>${body}
  </section>`;
}

function featuredSectionHtml(title, body, accent = '#345a73') {
  return `<section style="margin:28px 0 0;padding:0 0 24px 15px;border-left:3px solid ${accent};border-bottom:1px solid #e3e8ec">
    <h2 style="margin:0 0 12px;color:${accent};font-size:17px;line-height:1.4;font-weight:700">${escapeHtml(title)}</h2>${body}
  </section>`;
}
...
function collectionHtml(items, english, formatter = renderFinding) {
  ...
  if (!entries.length) return '<p style="margin:0;color:#697785">—</p>';
  return entries.map(({ title, details }, index) => `<div style="padding:12px 0${index === entries.length - 1 ? '' : ';border-bottom:1px solid #edf0f2'}">
    <strong style="color:#263f52;font-size:14px;line-height:1.55">${escapeHtml(title)}</strong>
    <div style="margin-top:5px;color:#586775;white-space:pre-line;line-height:1.7">${escapeHtml(details)}</div>
  </div>`).join('');
}
...
export function buildReportCompletionEmail({ audit, report, reportUrl }) {
  ...
  const briefBody = [
    sectionHtml(copy.summary, `<p style="margin:0;color:#3d4b58;line-height:1.8">${escapeHtml(asText(report?.newsSummary, '—'))}</p>`),
    featuredSectionHtml(copy.conclusion, `<p style="margin:0;font-size:17px;font-weight:700;line-height:1.65;color:#263f52">${escapeHtml(asText(report?.oneSentenceConclusion, '—'))}</p>`),
    sectionHtml(copy.value, `<p style="margin:0 0 6px;font-weight:700;color:#26343f">${escapeHtml(exportLocalizedValue(audit?.reportLanguage, asText(report?.readingValue, '—')))}</p><p style="margin:0;color:#586775;line-height:1.75">${escapeHtml(asText(report?.readingValueReason, '—'))}</p>`),
    sectionHtml(copy.readerGuide, `<p style="margin:0;color:#3d4b58;line-height:1.8">${escapeHtml(asText(report?.normalReaderGuide, '—'))}</p>`),
  ].join('');

  const reportBody = [
    sectionHtml(copy.scores, `<table role="presentation" style="width:100%;border-collapse:collapse">${scoreRowsList.map(({ label, value }, index) => `<tr><td style="padding:8px 0;color:#586775${index === scoreRowsList.length - 1 ? '' : ';border-bottom:1px solid #edf0f2'}">${escapeHtml(label)}</td><td style="padding:8px 0;text-align:right;font-weight:700;color:#26343f${index === scoreRowsList.length - 1 ? '' : ';border-bottom:1px solid #edf0f2'}">${value}/100</td></tr>`).join('')}</table>`),
  ...
  const metadataHtml = `<div style="margin:28px 0 0;padding:15px 0;border-top:1px solid #dfe5e9;border-bottom:1px solid #dfe5e9">
    <div style="margin:0 0 9px;color:#345a73;font-size:12px;font-weight:700;letter-spacing:.08em;text-transform:uppercase">${escapeHtml(copy.metadata)}</div>
    <table role="presentation" style="width:100%;border-collapse:collapse;table-layout:fixed">${metadataRows.map(([label, value]) => `<tr><td style="padding:5px 12px 5px 0;color:#697785;width:38%;vertical-align:top">${escapeHtml(label)}</td><td style="padding:5px 0;color:#26343f;vertical-align:top;overflow-wrap:anywhere">${escapeHtml(asText(value, '—'))}</td></tr>`).join('')}</table>
  </div>`;
  const text = [
    `${copy.subject}: ${title}`,
  ...
  return {
    ...
    text,
    html: `<div style="margin:0;padding:20px 12px;background:#eef2f5;font-family:Arial,'PingFang SC','Microsoft YaHei',sans-serif;color:#2f3d48;line-height:1.65">
      <main style="max-width:700px;margin:0 auto;background:#ffffff;border:1px solid #dce3e8;border-radius:12px;overflow:hidden">
        <header style="padding:25px 28px 23px;border-bottom:4px solid #8ca8ba;background:#ffffff"><div style="font-size:12px;letter-spacing:.12em;color:#5d7485;font-weight:700">GUANYU / 观隅</div><h1 style="margin:10px 0 7px;color:#263f52;font-size:25px;line-height:1.35">${escapeHtml(copy.heading)}</h1><p style="margin:0;color:#697785;line-height:1.7;overflow-wrap:anywhere">${escapeHtml(title)}</p></header>
        <div style="padding:24px 28px 30px"><p style="margin:0;color:#586775;line-height:1.8">${escapeHtml(copy.intro)}</p>${briefBody}${metadataHtml}${reportBody}<p style="margin:28px 0 0;text-align:left"><a href="${escapeHtml(reportUrl)}" style="display:inline-block;padding:10px 16px;border-radius:7px;background:#345a73;color:#ffffff;text-decoration:none;font-weight:700">${escapeHtml(copy.reportLink)}</a></p></div>
      </main>
    </div>`,
  };
}
