...
export function buildReportCompletionEmail({ audit, report, reportUrl }) {
  ...
  const sourceInterpretation = report?.sourceInterpretation || {};
  ...
  const reportBody = [
    ...
    sectionHtml(copy.value, `<p style="margin:0 0 6px;font-weight:700;color:#2d251b">${escapeHtml(asText(report?.readingValue, '—'))}</p><p style="margin:0;color:#5f5546;line-height:1.75">${escapeHtml(asText(report?.readingValueReason, '—'))}</p>`),
  ...
  const text = [
    ...
    sectionText(copy.value, `${asText(report?.readingValue, '—')}\n${asText(report?.readingValueReason, '—')}`),
