...
export function exportLocalizedValue(language, value) {
  ...
  const locale = normalizeExportLanguage(language);
  if (locale === 'zh-CN') return VALUE_ALIASES[raw] ? ({ source_supported: '原文支持', externally_verified: '外部已核验', partially_supported: '部分支持', pending_verification: '待核验', unable_to_verify: '暂无法确认', high: '高', medium: '中', low: '低' })[VALUE_ALIASES[raw]] : raw;
  if (locale === 'en-US') return ({ source_supported: 'Supported by the article', externally_verified: 'Externally verified', partially_supported: 'Partially supported', pending_verification: 'Pending verification', unable_to_verify: 'Unable to verify', high: 'High', medium: 'Medium', low: 'Low' })[VALUE_ALIASES[raw] || raw] || raw;
