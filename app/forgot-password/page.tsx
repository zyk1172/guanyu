        ...
        <form onSubmit={(event) => { event.preventDefault(); void resetPassword(); }} className="space-y-4">
          ...
          <label className="grid gap-1 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
            ...
            <div className="flex gap-2">
              <input value={captchaAnswer} onChange={(event) => setCaptchaAnswer(event.target.value)} placeholder={t('auth.captchaPlaceholder')} className="min-w-0 flex-1 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm normal-case tracking-normal text-gray-900 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 dark:border-gray-800 dark:bg-gray-900 dark:text-white" />
              <button type="button" onClick={refreshCaptcha} className="rounded-lg border border-gray-200 bg-gray-50 px-2 text-xs font-bold normal-case tracking-normal text-gray-600 hover:bg-gray-100 dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300" title={t('auth.refreshCaptcha')}>
                {captchaImage ? <>
