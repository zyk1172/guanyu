...
export async function PATCH(request: NextRequest) {
  try {
    ...
    try {
      assertValidPasswordInput(newPassword);
    } catch (error: any) {
      return NextResponse.json({ error: error?.message || '新密码长度不符合要求。' }, { status: 400 });
    }
    ...
    if (!account || !(await verifyPassword(currentPassword, account.password)).valid) {
