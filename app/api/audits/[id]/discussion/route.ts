...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ locked: Boolean(result.audit.discussionLockedAt), messages, canModerate: isAdmin, currentUserId: user?.id || null, nextCursor: messages.length === 20 ? messages[0].createdAt.toISOString() : null });
}
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ message: result.message, repeated: result.repeated, balance: effectiveCreditCents(await prisma.user.findUniqueOrThrow({ where: { id: user.id }, select: { creditBalance: true, creditBalanceCents: true, creditBalanceAmount: true } })) / 100 });
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json({ message: await prisma.reportDiscussionMessage.update({ where: { id: message.id }, data: { content: cleanContent(body.content), isEdited: true, editedAt: new Date() } }) });
}
